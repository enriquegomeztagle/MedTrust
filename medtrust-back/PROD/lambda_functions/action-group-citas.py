...
def consultar_cita(event):
    try:
        numeroCita = next(param['value'] for param in event.get('parameters', []) if param['name'] == 'numeroCita')
    ...
    if numeroCita == "1234":
        ...
        logger.info("Información dummy devuelta: %s", json.dumps(dummy_info, indent=2, ensure_ascii=False))
    else:
        dummy_info = {
            "estadoCita": "No encontrada",
            "mensaje": "El número de cita proporcionado no corresponde a ninguna cita registrada."
        }
        logger.info("Información para cita no encontrada: %s", json.dumps(dummy_info, indent=2, ensure_ascii=False))

    responseBody = {
        "TEXT": {
            "body": "Información de la cita: %s" % json.dumps(dummy_info, ensure_ascii=False)
        }
    }
...
def generar_cita(event):
    try:
        nombrePaciente = next(param['value'] for param in event.get('parameters', []) if param['name'] == 'nombrePaciente')
        telefono = next(param['value'] for param in event.get('parameters', []) if param['name'] == 'telefono')
        fechaCita = next(param['value'] for param in event.get('parameters', []) if param['name'] == 'fechaCita')
        horaCita = next(param['value'] for param in event.get('parameters', []) if param['name'] == 'horaCita')
        motivo = next(param['value'] for param in event.get('parameters', []) if param['name'] == 'motivo')
        
        logger.info("Datos recibidos: nombrePaciente=%s, telefono=%s, fechaCita=%s, horaCita=%s, motivo=%s", 
                    nombrePaciente, telefono, fechaCita, horaCita, motivo)
    ...
    responseBody = {
        "TEXT": {
            "body": "Cita creada exitosamente: %s\n\n%s" % (json.dumps(cita_info, ensure_ascii=False), mensaje_adicional)
        }
    }
...
def construir_respuesta(event, responseBody):
    ...
    logger.info("Respuesta final: %s", json.dumps(dummy_function_response, indent=2, ensure_ascii=False))
